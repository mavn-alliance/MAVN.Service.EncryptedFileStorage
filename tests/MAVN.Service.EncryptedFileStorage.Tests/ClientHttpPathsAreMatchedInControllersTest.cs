...
namespace MAVN.Service.EncryptedFileStorage.Tests
{
    public class ClientHttpPathsAreMatchedInControllersTest
    {
        ...
        {
            var excluded  = new List<string> { "IEncryptedFileStorageApi" };
            var clientInterface = typeof(IEncryptedFileStorageClient);

            var apiInterfaces = clientInterface
                .GetProperties()
                .Where(p => 
                    p.CanRead && p.PropertyType.IsInterface && !excluded.Contains(p.PropertyType.Name))
